...
public class ProfileQueryServiceTest {
  ...
  void should_find_profile_by_username() {
    UserData userData = new UserData("target-id", "target@test.com", "targetuser", "bio", "image");
    when(userReadService.findByUsername("targetuser")).thenReturn(userData);
    when(userRelationshipQueryService.isUserFollowing(currentUser.getId(), "target-id"))
        .thenReturn(true);
  ...
  @Test
  void should_return_empty_when_username_not_found() {
  ...
  @Test
  void should_find_profile_not_following() {
    UserData userData = new UserData("target-id", "target@test.com", "targetuser", "bio", "image");
    when(userReadService.findByUsername("targetuser")).thenReturn(userData);
    when(userRelationshipQueryService.isUserFollowing(currentUser.getId(), "target-id"))
        .thenReturn(false);
