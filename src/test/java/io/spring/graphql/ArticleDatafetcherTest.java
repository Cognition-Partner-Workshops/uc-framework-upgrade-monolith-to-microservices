...
public class ArticleDatafetcherTest {
  ...
  @Test
  void should_throw_not_found_when_user_not_found_for_feed() {
    try (MockedStatic<SecurityUtil> securityUtil = mockStatic(SecurityUtil.class)) {
      securityUtil.when(SecurityUtil::getCurrentUser).thenReturn(Optional.of(user));
  ...
  void should_get_articles_with_first_and_filters() {
    try (MockedStatic<SecurityUtil> securityUtil = mockStatic(SecurityUtil.class)) {
      ...
      CursorPager<ArticleData> pager =
          new CursorPager<>(Arrays.asList(articleData), Direction.NEXT, false);
      when(articleQueryService.findRecentArticlesWithCursor(
              eq("java"), eq("testuser"), eq(null), any(), eq(user)))
          .thenReturn(pager);
  ...
  void should_get_article_from_payload() {
    try (MockedStatic<SecurityUtil> securityUtil = mockStatic(SecurityUtil.class)) {
      ...
      DataFetchingEnvironment dataFetchingEnv = mock(DataFetchingEnvironment.class);
  ...
  @Test
  void should_throw_not_found_when_article_not_found_in_payload() {
    try (MockedStatic<SecurityUtil> securityUtil = mockStatic(SecurityUtil.class)) {
      securityUtil.when(SecurityUtil::getCurrentUser).thenReturn(Optional.of(user));
      ...
      assertThrows(
          ResourceNotFoundException.class, () -> articleDatafetcher.getArticle(dataFetchingEnv));
    }
  }
  ...
  void should_throw_not_found_when_comment_article_not_found() {
    try (MockedStatic<SecurityUtil> securityUtil = mockStatic(SecurityUtil.class)) {
      ...
      assertThrows(
          ResourceNotFoundException.class,
          () -> articleDatafetcher.getCommentArticle(dataFetchingEnv));
    }
  }
  ...
  void should_throw_not_found_when_slug_not_found() {
    try (MockedStatic<SecurityUtil> securityUtil = mockStatic(SecurityUtil.class)) {
      securityUtil.when(SecurityUtil::getCurrentUser).thenReturn(Optional.empty());
      when(articleQueryService.findBySlug(eq("nonexistent"), eq(null)))
          .thenReturn(Optional.empty());
  ...
  void should_get_feed_with_empty_result() {
    try (MockedStatic<SecurityUtil> securityUtil = mockStatic(SecurityUtil.class)) {
      ...
      when(articleQueryService.findUserFeedWithCursor(eq(null), any())).thenReturn(emptyPager);
  ...
  @Test
  void should_get_feed_with_cursor_after() {
    try (MockedStatic<SecurityUtil> securityUtil = mockStatic(SecurityUtil.class)) {
      securityUtil.when(SecurityUtil::getCurrentUser).thenReturn(Optional.of(user));
      ...
      DataFetcherResult<ArticlesConnection> result =
          articleDatafetcher.getFeed(10, "1672531200000", null, null, dfe);
