...
public class ArticleBookmarkApiTest extends TestWithCurrentUser {
  ...
  @Test
  public void should_get_bookmarked_articles() throws Exception {
    when(articleBookmarkRepository.findBookmarkedArticleIds(eq(user.getId())))
        .thenReturn(Collections.singletonList(article.getId()));
    given()
        .header("Authorization", "Token " + token)
        .when()
  ...
  @Test
  public void should_get_empty_bookmarked_articles() throws Exception {
    when(articleBookmarkRepository.findBookmarkedArticleIds(eq(user.getId())))
  ...
  @Test
  public void should_fail_bookmark_without_auth() throws Exception {
    given().when().post("/articles/{slug}/bookmark", article.getSlug()).then().statusCode(401);
  }

  @Test
  public void should_fail_bookmark_article_not_found() throws Exception {
    when(articleRepository.findBySlug(eq("nonexistent"))).thenReturn(Optional.empty());
    given()
        .header("Authorization", "Token " + token)
        .when()
        .post("/articles/{slug}/bookmark", "nonexistent")
        .then()
        .statusCode(404);
  }
}
