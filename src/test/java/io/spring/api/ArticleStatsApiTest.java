...
public class ArticleStatsApiTest extends TestWithCurrentUser {
  ...
  public void should_get_article_stats_success() throws Exception {
    ...
    ArticleStatsData stats = new ArticleStatsData(slug, "Test Article", 0, 5, 3, 7);
  ...
  public void should_404_if_article_not_found_for_stats() throws Exception {
    when(articleStatsQueryService.getArticleStats(eq("not-exists")))
        .thenReturn(Optional.empty());

    RestAssuredMockMvc.when()
        .get("/articles/{slug}/stats", "not-exists")
        .then()
        .statusCode(404);
  }
  ...
  public void should_get_article_stats_with_zero_counts() throws Exception {
    ...
    ArticleStatsData stats = new ArticleStatsData(slug, "New Article", 0, 0, 0, 0);
