...
async def spy_page(request: Request):
    ...
    try:
        test_cases = get_all_test_cases(db)
        tc_list = [{"id": tc.id, "name": tc.name, "base_url": tc.base_url or ""} for tc in test_cases]
    finally:
        db.close()
    return templates.TemplateResponse("object_spy.html", {
        "request": request,
        "user": get_current_user(request),
        "test_cases": tc_list,
    })
...
@router.post("/spy/inspect")
async def spy_inspect(request: Request):
    """Inspect a single element by selector on a given URL."""
    require_auth(request)
    body = await request.json()
    url = body.get("url", "")
    ...
    if not url or not selector:
        return JSONResponse({"error": "url and selector are required"}, status_code=400)

    from agent.object_spy import object_spy
    ...
    try:
        with sync_playwright() as p:
            ...
            element = object_spy.inspect_element(page, selector)
            browser.close()

        if element:
            return JSONResponse(element.to_dict())
        return JSONResponse({"error": "Element not found"}, status_code=404)
    except Exception as e:
        return JSONResponse({"error": str(e)}, status_code=500)


@router.post("/spy/page-scan")
async def spy_page_scan(request: Request):
    """Bulk scan all interactive elements on a page."""
    require_auth(request)
    body = await request.json()
    url = body.get("url", "")
    ...
    try:
        with sync_playwright() as p:
            ...
            page = browser.new_page()
            page.goto(url, wait_until="domcontentloaded", timeout=15000)
            elements = object_spy.inspect_page(page, scope)
            browser.close()
...
@router.post("/spy/suggest-steps")
async def spy_suggest_steps(request: Request):
    """Generate suggested test steps from captured elements."""
    require_auth(request)
    body = await request.json()
    url = body.get("url", "")
    ...
    try:
        with sync_playwright() as p:
            ...
            page = browser.new_page()
            page.goto(url, wait_until="domcontentloaded", timeout=15000)
            elements = object_spy.inspect_page(page)
            suggestions = object_spy.suggest_test_steps(elements)
            browser.close()

        return JSONResponse({"steps": suggestions, "element_count": len(elements)})
    except Exception as e:
        return JSONResponse({"error": str(e)}, status_code=500)


@router.post("/spy/export-pom")
async def spy_export_pom(request: Request):
    """Export captured elements as a Page Object Model Python file."""
    require_auth(request)
    body = await request.json()
    url = body.get("url", "")
    ...
    try:
        with sync_playwright() as p:
            ...
            page.goto(url, wait_until="domcontentloaded", timeout=15000)
            elements = object_spy.inspect_page(page)
            pom_code = object_spy.export_locators(elements, format="page_object")
            browser.close()

        return JSONResponse({"code": pom_code, "element_count": len(elements)})
    except Exception as e:
        return JSONResponse({"error": str(e)}, status_code=500)
...
async def healing_report(request: Request):
    """Get the auto-healing report for the current session."""
    require_auth(request)
    from agent.auto_healer import auto_healer
    return JSONResponse({"events": auto_healer.get_healing_report()})


@router.post("/healing/clear")
async def healing_clear(request: Request):
    """Clear the auto-healing log."""
    require_auth(request)
    from agent.auto_healer import auto_healer
    auto_healer.clear_log()
    return JSONResponse({"status": "cleared"})


@router.post("/selector/score")
async def selector_score(request: Request):
    """Score a selector for stability (0.0 - 1.0)."""
    body = await request.json()
    selector = body.get("selector", "")
    from agent.auto_healer import score_selector_stability
    score = score_selector_stability(selector)
