CREATE TABLE products (
    ...
    category VARCHAR(30) NOT NULL, risk_level VARCHAR(20) NOT NULL,
    expected_return_min DECIMAL(5,2), expected_return_max DECIMAL(5,2),
    min_investment DECIMAL(15,2) DEFAULT 0, max_investment DECIMAL(15,2),
    tax_benefit_section VARCHAR(20), features JSONB DEFAULT '{}',
    is_active BOOLEAN NOT NULL DEFAULT TRUE, priority INT DEFAULT 0,
...
CREATE TABLE product_risk_mappings (
    ...
    risk_category VARCHAR(20) NOT NULL, allocation_percent DECIMAL(5,2) NOT NULL, priority INT DEFAULT 0
);
...
INSERT INTO products (id, name, description, category, risk_level, expected_return_min, expected_return_max, min_investment, tax_benefit_section) VALUES
    (gen_random_uuid(), 'High-Yield Savings Account', 'Premium savings with competitive interest', 'SAVINGS_ACCOUNT', 'LOW', 3.5, 5.0, 10000, NULL),
    ...
    (gen_random_uuid(), 'ELSS Tax Saver Fund', 'Equity linked savings with tax benefits', 'TAX_SAVER', 'MODERATE', 10.0, 15.0, 500, '80C'),
    (gen_random_uuid(), 'National Pension System', 'Government-backed pension scheme', 'PENSION', 'LOW', 8.0, 10.0, 1000, '80CCD'),
    (gen_random_uuid(), 'Sovereign Gold Bond', 'Government securities in gold', 'GOLD', 'LOW', 6.0, 10.0, 5000, NULL),
    (gen_random_uuid(), 'PPF Account', 'Public Provident Fund with tax-free returns', 'PPF', 'LOW', 7.0, 7.5, 500, '80C'),
    (gen_random_uuid(), 'Small Cap Equity Fund', 'High-growth small cap fund', 'MUTUAL_FUND', 'VERY_HIGH', 15.0, 25.0, 5000, NULL),
