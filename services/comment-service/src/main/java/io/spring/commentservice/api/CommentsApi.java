...
import io.spring.commentservice.client.ArticleServiceClient.ArticleInfo;
...
public class CommentsApi {
  ...
  public ResponseEntity<?> createComment(
    ...
    return ResponseEntity.status(201)
        .body(commentResponse(commentQueryService.findById(comment.getId())
            .orElseThrow(ResourceNotFoundException::new)));
  }

  @GetMapping
  public ResponseEntity getComments(
      @PathVariable("slug") String slug, @AuthenticationPrincipal Object user) {
    ArticleInfo article =
        articleServiceClient
            .getArticleBySlug(slug)
            .orElseThrow(ResourceNotFoundException::new);
    List<CommentData> comments = commentQueryService.findByArticleId(article.getId());
        ...
        .map(
            comment -> {
              if (!AuthorizationService.canWriteComment(
                  userId, article.getUserId(), comment)) {
